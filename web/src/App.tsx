  ...
  return (
    <div className="max-w-[1344px] mx-auto flex flex-col items-center my-20">
      ...
      <div className="grid grid-cols-6 gap-6">
       {games.map(game => {
        return (
          <GameBanner key = {game.id} bannerUrl={game.bannerUrl} title={game.title} adCount={game._count.Ad} />
        )
       })}
      </div>
            ...
            <Dialog.Title className="text-3xl font-black">Publique um anúncio</Dialog.Title>
              <form className="mt=8 flex flex-col gap-4">
                <div className="flex flex-col gap-2">
                  ...
                  <Input id="game" placeholder="Selecione o game que dejesa jogar" />
                </div>
                <div className="flex flex-col gap-2">
                ...
                <div className="grid grid-cols-2 gap-6">
                  <div className="flex flex-col gap-2">
                    ...
                    <Input id="yearsPlaying" type="number" placeholder="Pode ser zero" />
                  </div>
                  <div className="flex flex-col gap-2">
                    <label htmlFor="discord">Qual seu discord?</label>
                    <Input id="discord" type="text" placeholder="Usuario#123"/>
                  </div>
                  ...
                  <footer className=" mt-4 flex justify-end gap-4">
                    <Dialog.Close type="button" className="bg-zinc-500 px-5 h-12 rounded-md font-semibold hover:bg-zinc-800">Cancelar</Dialog.Close>
                    <button className="bg-violet-500 p-5 h-12 rounded-md font-semibold flex items-center gap-3 hover:bg-violet-800" type="submit"><GameController/>Encontrar duo</button>
                  </footer>
              </form>
            </Dialog.Content>
        </Dialog.Portal>
